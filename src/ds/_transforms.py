...
def imagenet_transform(resize_size=224,
                       crop_size=224,
...
#             probability=0.5, min_area=0.1, max_area=1 / 2, min_aspect=0.3, max_aspect=None,
    ...
    def _erase(self, input, chan, img_h, img_w, dtype):
    ...
    def __call__(self, input):
        if len(input.size()) == 3:
            input_masked, is_masked = self._erase(input, *input.size(), input.dtype)
        else:
...
#                 ?? = self._erase(input[i], chan, img_h, img_w, input.dtype)
        return input, input_masked, is_masked
